      ...
      (cases expression exp
        ...
        (raise-exp (exp1)
          (value-of/k exp1 env
            (raise1-cont cont)))
        (letcc-exp (id exp)
                   (value-of/k exp (extend-env id (cont-val cont) env) cont)))))
    ...
    (lambda (cont val)
      (cases continuation cont
        ...
        (rand-cont (val1 saved-cont)
          (cases expval val1
            (proc-val (proc) (apply-procedure proc val saved-cont))
            (cont-val (cc) (apply-cont cc val))
            (else (eopl:error rand-cont "Expected cont or proc got " val1))))
