(module interp (lib "eopl.ss" "eopl")
      ...
      (cases expression exp
        ...
        (newarray-exp (exp1 exp2)
                      (let ((size (expval->num (value-of exp1 env)))
                            (init (expval->num (value-of exp2 env))))
                        (array-val (make-array size init))))
        
        (arrayref-exp (exp1 exp2)
                      (let ((id (value-of exp2 env))
                            (array (value-of exp1 env)))
                        (arrayref array id)))
        (arrayset-exp (exp1 exp2 exp3)
                      (let ((id (value-of exp2 env))
                            (array (value-of exp1 env))
                            (val (value-of exp3 env)))
                        (arrayset array id val)))
  ...
  (define print-array
    (lambda (array i)
      (if (not (eqv? i 0))
          (begin 
            (write (expval->num (arrayref (expval->array array) i)))
  ...
  (print-array (value-of-program (scan&parse "newarray(10, 11)")) 10)
