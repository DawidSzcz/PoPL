(module interp (lib "eopl.ss" "eopl")
      ...
      (cases expression exp
        ...
        (list-exp (exps)
                  (if (null? exps) 
                      ...
                      (value-of/k (car exps) env (list-cont (list-val (cdr exps)) env cont)))))))
  ...
  (define apply-cont
    (lambda (cont val)
      (cases continuation cont
        ...
        (list-cont (restval saved-env saved-cont)
                   (let ((rest (expval->list restval)))
                     (if (null? rest)
                         (apply-cont saved-cont (list-val (list val)))
                         (value-of/k (car rest) 
                                     saved-env 
                                     (list-cont (list-val (cdr rest)) saved-env (cons-cont saved-cont val))))))
        (cons-cont (saved-cont el)
                   (apply-cont saved-cont (list-val (cons el (expval->list val)))))
        )))
  ...
  (value-of-program (scan&parse "let x = list(13, 42) in x"))
